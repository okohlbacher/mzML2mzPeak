    ...
    > ChromatogramSource for ImzMLReaderType<R, S, C, D>
{
    ...
    // VENDORED PATCH (plan 00-01): the published mzdata 0.63.3 imzML reader omits this
    // required `ChromatogramSource` method, causing E0046. imzML has no chromatograms,
    // so the count is always 0. Mirrors unpublished mzdata master (reader.rs:1182).
