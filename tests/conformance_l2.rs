...
fn numpress_archive_carries_transform_record_in_both_places() {
    ...
    convert_mzml(input, &out, &EncodingOptions::compact(), None, None).expect("numpress conversion");
...
fn lossless_mz_passes_l1_and_carries_no_transform_block() {
    ...
    let _ = std::fs::remove_file(&out);
    convert_mzml(input, &out, &EncodingOptions::lossless(), None, None).expect("lossless conversion");
