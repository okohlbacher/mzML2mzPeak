...
fn tiny_pwiz_mzml_converts_and_reads_back() {
    ...
    let report = convert_mzml(input, &out, &mzml2mzpeak::write::EncodingOptions::default()).expect("convert_mzml must succeed");
...
fn produced_archive_is_always_openable() {
    ...
    let _ = std::fs::remove_file(&out);
    convert_mzml(input, &out, &mzml2mzpeak::write::EncodingOptions::default()).expect("convert");
    assert!(
