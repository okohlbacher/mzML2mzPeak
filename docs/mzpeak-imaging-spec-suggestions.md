...
>
> **[GEOF-01] Declared-vs-observed consistency rule.** When a source imzML carries a `<scanSettings>` grid (`IMS:1000042/43`), the converter _SHOULD_ honour the declared count as authoritative — recording `pixel_count_source: "declared"` — **only when** the declared grid is *consistent* with the observed pixel coordinates: all observed coordinates lie within the declared bounds (observed max ≤ declared on both x and y). When observed coordinates **exceed** the declared grid on any axis, the converter _MUST NOT_ trust the declared count; it _MUST_ record `pixel_count_source: "observed_max"` using the actual observed extent, and _SHOULD_ emit a counted, non-fatal warning naming the declared-vs-observed discrepancy. This rule prevents fabricating a declared pixel count that the data contradicts (a declared 260×134 grid with a pixel at (270, 1) would be wrong). The happy declared path — where the observed extent equals or lies within the declared grid — converts to `pixel_count_source: "declared"` with no warning, as the grid declaration is a reliable authority. **Implemented in** `src/write/writer.rs::IndexAccumulator::fold_into` (GEOF-01 consistency guard) and end-to-end proven by `tests/geometry_roundtrip.rs::declared_geometry_roundtrip` on the `Synthetic_DeclaredGrid` fixture.
