...
| `tests/reverse_read_spike.rs` | `imzml2mzpeak::reverse::ReverseError` | import + `matches!(err, ReverseError::NotImaging)` assertion | VERIFIED | Line 37: `use imzml2mzpeak::reverse::ReverseError;`. Line 299: `assert!(matches!(err, ReverseError::NotImaging), ...)`. |
...
| SHA-1 is a direct dep of imzml2mzpeak | `cargo tree -i sha1` | `sha1 v0.10.6` with `imzml2mzpeak v0.1.0` as first-level dependent | PASS |
| MD5 (RustCrypto) is a direct dep | `cargo tree -i md-5` | `md-5 v0.10.6` with `imzml2mzpeak v0.1.0` as direct dependent (only consumer) | PASS |
| md5 v0.7.0 is transitive only (via mzdata) | `cargo tree -i md5` | `md5 v0.7.0` → `mzdata v0.63.3` only; `imzml2mzpeak` is NOT a direct consumer | PASS |
